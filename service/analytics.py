...
def minerar_regras(df_analise):
    ...
    if itens_frequentes.empty:
        print("⚠️ [Analytics] Nenhum item frequente encontrado com este suporte.")
        return None
        
    print(f"⚖️ [Analytics] Gerando regras de associação (Confiança Min: {config.CONFIANCA_MIN})...")
    regras = association_rules(itens_frequentes, metric="confidence", min_threshold=config.CONFIANCA_MIN)
    
    if regras.empty:
        print("⚠️ [Analytics] Nenhuma regra gerada com esta confiança.")
        return None
